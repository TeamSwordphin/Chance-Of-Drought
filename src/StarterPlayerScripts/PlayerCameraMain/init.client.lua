		...
		if accessory:IsA("Accessory") then
			for _, part in ipairs(accessory:GetDescendants()) do
				if part:IsA("BasePart") then
					TweenService:Create(part, TweenInfo.new(0.5), { LocalTransparencyModifier = transparencyModifier })
						:Play()
				end
			end
		end
...
local function mainCamera(delta)
	...
	local shoulder = isFP and script:GetAttribute("OffsetFirstPersonShoulder") or script:GetAttribute("OffsetShoulder")
	...
	local center = TargetRoot.Position
		+ Vector3.new(0, isFP and script:GetAttribute("BaseFirstPersonHeight") or script:GetAttribute("BaseHeight"), 0)
	...
	if script:GetAttribute("PopperCamEnabled") then
		local raycastResult: RaycastResult = workspace:Raycast(center, cframe.Position - center, PopperRaycastParams)
		if raycastResult then
			position = raycastResult.Position + raycastResult.Normal * script:GetAttribute("PopperCamMinimumSpacing")
		end
	end
